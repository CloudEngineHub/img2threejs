...
def _dct_2d(block: list[list[float]]) -> list[list[float]]:
    ...
    # cols: out = temp @ M^T
    out = [[sum(temp[k][j] * m[frequency][j] for j in range(n)) for frequency in range(n)] for k in range(n)]
    return out
...
def phash(gray: list[list[float]], hash_size: int = 8) -> int:
    ...
    coeffs = _dct_2d(gray)
    low = [coeffs[k][frequency] for k in range(hash_size) for frequency in range(hash_size)]
